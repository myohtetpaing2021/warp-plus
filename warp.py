...
import base64
import json
# i Am S-S-B >>> t.me/shayshayblack 🫶  - Telegram Channel - t.me/shayshayblack 🫶
warp_cidr = [
    "162.159.192.0/24",
    "162.159.193.0/24",
    "162.159.195.0/24",
...
title = (
    ...
    + base64.b64encode("Warp+BySSB".encode("utf-8")).decode("utf-8")
    + "\n"
)
...
profile_web = "//profile-web-page-url: https://github.com/myohtetpaing2021\n"
...
def export_SingBox(t_ips):
    with open("assets/SSB-singbox-template.json", "r") as f:
        data = json.load(f)

    data["outbounds"][1]["outbounds"].extend(["🚀TEHRAN-SSB", "🚀BERLIN-SSB"])

    tehran_wg = toSingBox("🚀TEHRAN-SSB", t_ips[0], "direct")
    if tehran_wg:
        data["outbounds"].insert(2, tehran_wg)
    else:
        print("Failed to generate 🚀TEHRAN-SSB configuration")

    berlin_wg = toSingBox("🚀BERLIN-SSB", t_ips[1], "🚀TEHRAN-SSB")
    if berlin_wg:
        data["outbounds"].insert(3, berlin_wg)
    else:
        print("Failed to generate 🚀BERLIN-SSB configuration")
